<template>
    <div class="category-wrap">
        ...
        <div class="category-header">
            ...
            <!-- 필터 -->
            <!-- <sorting-wrap></sorting-wrap> -->
        </div>
        <!-- 카테고리 컨테이너 -->
        <div class="category-container">
            <ul class="category-wrapper">
                <goods-list v-for="(item,index) in itemList" :key="index"
...
<script>
    module.exports = {
        ...
        mounted(){
            ...
            // let CatobserverOption ={ threshold : 0.1}
            // let Catobserver = new IntersectionObserver((e)=>{
            //     e.forEach((header)=>{
            //         if(header.isIntersecting) {
            //             header.target.classList.add('fixed');
            //         }
            //         else{
            //             header.target.classList.remove('fixed');
            //         }
            //      })
            // });
            const categoryHeader = document.querySelector('.category-wrapper');
            categoryHeader.addEventListener('scroll', ()=>{
                console.log('zz')
            })
            // Catobserver.observe(categoryHeader);
...
.category-nav-list{ position: absolute; padding:12px 16px; width:100%; height:0; text-align: left; visibility: hidden; overflow: hidden; background: #fff; z-index: 10; border:1px solid #eee; transition: all .3s ease-in-out; } 
